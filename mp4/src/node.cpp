...
Node::Node(const std::string_view& host, const std::string_view& port, NodeId& introducer, Logger& logger) :
    socket(host, port), file_socket(host, FILE_PORT), tuple_socket(host, TUPLE_PORT), self(NodeId::createNewNode(host, port)), introducer(introducer), mem_list(logger), successor_list(N_successors, self, logger), file_system("./storage/local_storage", "./storage/HyDFS_storage", logger), stream_system(introducer, std::string(host)), logger(logger), fd_mode(PINGACK){
            ...
            while (true) {
                ...
                switch (message.type) {
                  ...
                  case FileMessageType::GREP_DONE:
                    ...
                    break;
                  case FileMessageType::AGGRE_APPEND_REQ:
                    handle_AggreAppendReq(message);
                    break;
                  default:
...
void Node::handleIncomingStreamMsg() {

    while (!left) {
        // Wait for a new connection
        struct sockaddr_in client_addr;
        int client_fd = stream_system.stream_socket.acceptClient(client_addr);
        if (client_fd < 0) {
            std::cerr << "[TCP] accept() failed\n";
            continue;
        }
        ...
        std::thread([this, client_fd]() {
            ...
            // 1. Read 4-byte message length header
            uint32_t len_net;
            // Use MSG_WAITALL to ensure we read the full 4 bytes if the connection is active
            ssize_t hdr = recv(client_fd, &len_net, sizeof(len_net), MSG_WAITALL);
            
            // Check for connection closed (hdr == 0) or error (hdr < 0)
            if (hdr != sizeof(len_net)) {
                // If the connection was closed before sending the header, we just exit.
                stream_system.stream_socket.closeConnection(client_fd);
                return; 
            }
            ...
            if (msg_len == 0 || msg_len > TCPSocketConnection::BUFFER_LEN * 8) {
                std::cerr << "[TCP] Invalid message length: " << msg_len << std::endl;
                stream_system.stream_socket.closeConnection(client_fd);
                return;
            }
            ...
            if (body != msg_len) {
                std::cerr << "[TCP] Error reading full body. Expected: " << msg_len 
                          << ", Read: " << body << ". Connection may be closed prematurely." << std::endl;
                stream_system.stream_socket.closeConnection(client_fd);
                return;
            }
            ...
            try {
                ...
                switch (message.type) {
                  case StreamMessageType::STARTTASK_REQ:
                    stream_system.handle_StartTask_Req(message);
                    break;
                  case StreamMessageType::RoutingTable_Req:
                    stream_system.handle_RoutingTable_Req(message);
                    break;
                  case StreamMessageType::LIST_REQ:
                    stream_system.handle_List_Req();
                    break;
                  case StreamMessageType::READY:
                    stream_system.handle_Ready_Req(message);
                    break;
                  case StreamMessageType::GET_CHECKPOINT:
                    get(std::string(message.src_dir), std::string(message.dst_file)); // reuse these var
                    break;
                  case StreamMessageType::KILL_RESTART_REQ:
                    stream_system.handle_kill_restart_req(message);
                    break;
                  case StreamMessageType::LOAD_REPORT:
                    stream_system.handle_load_report(message);
                    break;
                  case StreamMessageType::KILL_REQ:
                    stream_system.handle_kill_req(message);
                    break;
                  case StreamMessageType::RESET_REQ:
                    stream_system.handle_reset_req();
                    break;
                  default: 
                    std::cerr << "[TCP] Unknown StreamMessageType: "
                              << static_cast<int>(message.type) << std::endl;
                    break;
                }
            } catch (const std::runtime_error& e) {
                // Catches deserialization errors (like "Buffer too small...")
                std::cerr << "[TCP] Deserialization failed for message of length " << msg_len
                          << ": " << e.what() << std::endl;
            } catch (...) {
                std::cerr << "[TCP] An unknown error occurred during message processing." << std::endl;
            }

            // 5. Clean up the connection and thread
            stream_system.stream_socket.closeConnection(client_fd);
        }).detach();
    }
}
...
void Node::handleIncomingTuples() {
    // Initialize TCP server (bind + listen)
    ...
    while (!left) {
        ...
        std::thread([this, client_fd]() {
            while (true) {
                ...
                uint32_t msg_len = ntohl(len_net);
                if (msg_len > TCPSocketConnection::BUFFER_LEN * 8) {
                    std::cerr << "[NODE_TCP] Invalid message length: " << msg_len << std::endl;
                    break;
                }
                ...
                // Deserialize
                Tuple tuple = Tuple::deserialize(buffer.data(), msg_len);

                switch (tuple.type) {
                  case TupleMessageType::FINAL_OPUTPUT:
                    forward_tuple_to_file(tuple); 
                    break;
                  case TupleMessageType::AGGREGATE_OUTPUT:
                    forward_tuple_to_file_aggregate(tuple); 
                    break;
                  case TupleMessageType::INPUT_COMMIT:
                    handle_input_commit(tuple);
                    break;
                  case TupleMessageType::ACK_COMMIT:
                    handle_ack_commit(tuple);
                    break;
                  default:
                    std::cerr << "[NODE_TCP] Unknown TupleMessageType: "
                              << static_cast<int>(tuple.type) << std::endl;
                  break;
                }
...
//         // --- Handle this streaming connection in its own thread ---
//         std::thread([this, client_fd]() {
            
//             // CRITICAL FIX: Loop to process ALL tuples sent on this single connection
//             while (true) { 
//                 // 1. Read 4-byte message length header
//                 uint32_t len_net;
//                 ssize_t hdr = reliable_recv(client_fd, &len_net, sizeof(len_net));
                
//                 // Connection closed (hdr <= 0) or error. Break loop and close connection.
//                 if (hdr != sizeof(len_net)) {
//                     break; 
//                 }
...
//                 // Basic sanity check for message length
//                 if (msg_len == 0 || msg_len > TCPSocketConnection::BUFFER_LEN * 8) {
//                     std::cerr << "[NODE_TCP] Invalid tuple length: " << msg_len << std::endl;
//                     break;
//                 }
...
//                 // CRITICAL: Validate the body read
//                 if (body != msg_len) {
//                     std::cerr << "[NODE_TCP] Error reading full tuple body. Expected: " << msg_len 
//                               << ", Read: " << body << ". Sender likely disconnected." << std::endl;
//                     break;
//                 }
...
//                 } catch (const std::runtime_error& e) {
//                     std::cerr << "[NODE_TCP] Deserialization failed for tuple: " << e.what() << std::endl;
//                     // Note: If deserialization fails, the connection is corrupted. Break.
//                     break;
//                 } catch (...) {
//                     std::cerr << "[NODE_TCP] An unknown error occurred during tuple processing." << std::endl;
//                     break;
//                 }

//                 // Loop continues to read the next tuple's header...
//             } // end while(true) loop

//             // 4. Clean up the connection and thread (only executed after the sender closes)
//             tuple_socket.closeConnection(client_fd);
//         }).detach();
//     }
// }

void Node::runScalingManager() {

    while (true) {

        std::this_thread::sleep_for(std::chrono::milliseconds(3000)); 

        std::unordered_map<int, std::unordered_map<int, double>> current_loads_snapshot;
        
        {
            std::lock_guard<std::mutex> lock(stream_system.task_load_mtx);
            current_loads_snapshot = stream_system.stage_task_loads; 
        } 
        
        for (const auto& stage_pair : current_loads_snapshot) {
            int stage_id = stage_pair.first;
            const auto& task_loads = stage_pair.second;
            ...
            for (const auto& task_pair : task_loads) {
                total_load += task_pair.second;
            }

            size_t task_count = task_loads.size(); 
            double average_load = total_load / (double)task_count;
            
            std::cout << "[SCALING ANALYSIS] Stage " << stage_id 
                      << " Ave Load (" << task_count << " tasks): " 
                      << std::fixed << std::setprecision(2) << average_load << " tuples/sec." 
                      << std::endl;

            if (average_load > stream_system.high_watermark) {
                std::cout << "[SCALING DECISION] Stage " << stage_id << " is ABOVE HW (" 
                          << stream_system.high_watermark << "). SCALING UP." << std::endl;
                ...
                stream_system.increase_task(all_members, stage_id);
                std::this_thread::sleep_for(std::chrono::milliseconds(4000));
                
            } else if (average_load < stream_system.low_watermark) {
                 // CRITICAL CHECK: Only scale down if there is more than one task
                if (task_count > 1) { 
                    std::cout << "[SCALING DECISION] Stage " << stage_id << " is BELOW LW (" 
                              << stream_system.low_watermark << "). SCALING DOWN." << std::endl;
                    stream_system.decrease_task(stage_id);
                } else {
                     std::cout << "[SCALING INFO] Stage " << stage_id << " is below LW but cannot scale down (only 1 task remaining)." << std::endl;
                }
            }
        }
    }
...
void Node::send_file_chunk(const NodeId& target_node, const vector<FileChunkMessage>& chunks) {
  ...
  int sock = get_persistent_socket(target_node.host, FILE_PORT);
  if (sock < 0) {
      std::cerr << "[NODE] Failed to connect to " << target_node.host << std::endl;
      return;
  }
  ...
  for (const auto& chunk : chunks) {
      ...
      if (send(sock, &len, sizeof(len), 0) != sizeof(len)) {
          perror("[NODE] send length failed");
          continue;
      }
      ...
      if (written < 0) {
        ...
        close(sock);
        persistent_sockets.erase(std::string(target_node.host) + ":" + FILE_PORT); 
        perror("[NODE] send chunk failed");
      }
  }
...
int Node::get_persistent_socket(const std::string& host, const std::string& port) {
    ...
    int sock = file_socket.connectToServer(host, port);
    if (sock < 0) {
        cout << "[ERROR] Failed to connect to " << key;
        return -1;
    }

    persistent_sockets[key] = sock;
    // cout << "[CONNECT] Established persistent socket to " << key << endl;

    return sock;
}

void Node::startRainStorm(int Nstages, int Ntasks_per_stage, 
                          ...
                          bool exactly_once, bool autoscale_enabled, int INPUT_RATE, int LW, int HW ){
  auto all_members = mem_list.copy();
  std::sort(all_members.begin(), all_members.end(), [](const MembershipInfo& a, const MembershipInfo& b) {
      return std::string(a.node_id.host) < std::string(b.node_id.host);
  });
  NodeId target_node = file_to_node(dst_file);
  std::stringstream ss;
  ss << "[StreamSystem] Stream result would be on " << target_node.host;
  logger.log(ss.str());
  stream_system.startStream(all_members, Nstages, Ntasks_per_stage, op_exec, op_args, src_dir, dst_file, 
                          exactly_once, autoscale_enabled, INPUT_RATE, LW, HW);                     
}

void Node::list_tasks(){
  auto all_members = mem_list.copy();
  stream_system.list_tasks(all_members);                     
}

void Node::kill_restart_task(const std::string& vm_id, const std::string& task_id){
  stream_system.kill_restart_task(vm_id, task_id);            
}

void Node::reset_stream(){
  auto all_members = mem_list.copy();
  stream_system.reset(all_members);            
} 
