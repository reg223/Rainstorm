...
class Node {
 public:
  ...
  void handleIncomingStreamMsg();
  void handleIncomingTuples();
  void handleOutgoing();
  void runScalingManager();

  void startRainStorm(int Nstages, int Ntasks_per_stage, 
  ...
  void list_tasks(); // for stream system
 ...
 private:
  ...
  void handle_AggreAppendReq(FileChunkMessage& message);
  ...
  void forward_tuple_to_file(const Tuple&);
  void forward_tuple_to_file_aggregate(const Tuple& t);
  void handle_input_commit(const Tuple&);
  void handle_ack_commit(const Tuple&);
  ...
  std::unordered_map<std::string, int> aggregation_counts; // for tuple aggregate_by_key
